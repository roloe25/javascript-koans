 ...
 */
 ...
 //so many made functions that operate here
describe("About Higher Order Functions", function() {
  ...
  it("should use 'reduce' to update the same result on each iteration", function () {
    var numbers   = [1, 2, 3];
    //memo saves value and updates as it goes thru list
    //memo starts off at zero and then makes it self
    //the reason i wasnt able to do underbar beofre was because i just did the basics in the koans
    var reduction = _(numbers).reduce(
      //x is how i would like to modify it as
      function(memo, x) {
  ...
  it("should use range to generate an array", function() {
    //range uses a numerical index and has the 0 index at the value 0
  ...
  //using .chain makes it so much easier to read lets us use whitespace for readability, and it ends with .value()
  it("should use chain() ... .value() to use multiple higher order functions", function() {
    ...
    //chaining flatten, map, reduce,
    result = result.flatten()
    result = result.map((function(x) { return x+1 } ))
    result = result.reduce(function(sum, x) { return sum + x })
